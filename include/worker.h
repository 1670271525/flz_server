...
public:
    ...
    static WorkerGroup::ptr Create(uint32_t batch_size, flz::Scheduler* s = flz::Scheduler::GetThis()) {
        return std::make_shared<WorkerGroup>(batch_size, s);
    }

    WorkerGroup(uint32_t batch_size, flz::Scheduler* s = flz::Scheduler::GetThis());
    ~WorkerGroup();
...
public:
    ...
    void schedule(const std::string& name, FiberOrCb fc, int thread = -1) {
        ...
        if(s) {
            ...
            static flz::Logger::ptr s_logger = FLZ_LOG_NAME("system");
            FLZ_LOG_ERROR(s_logger) << "schedule name=" << name
                << " not exists";
        }
    }
...
typedef flz::Singleton<WorkerManager> WorkerMgr;
