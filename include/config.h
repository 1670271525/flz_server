...
public:
    ...
    std::string toString() override {
        try {
            ...
            FLZ_LOG_ERROR(FLZ_LOG_ROOT()) << "ConfigVar::toString exception "
    ...
    bool fromString(const std::string& val) override {
        try {
            ...
            FLZ_LOG_ERROR(FLZ_LOG_ROOT()) << "ConfigVar::fromString exception "
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != GetDatas().end()) {
            ...
            if(tmp) {
                FLZ_LOG_INFO(FLZ_LOG_ROOT()) << "Lookup name=" << name << " exists";
                return tmp;
            } else {
                FLZ_LOG_ERROR(FLZ_LOG_ROOT()) << "Lookup name=" << name << " exists but type not "
        ...
        if(name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678")
                != std::string::npos) {
            FLZ_LOG_ERROR(FLZ_LOG_ROOT()) << "Lookup name invalid " << name;
