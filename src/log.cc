...
#undef XX
	...
	flz::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
		flz::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

	struct LogIniter {
		LogIniter() {
			g_log_defines->addListener([](const std::set<LogDefine>& old_value,
						const std::set<LogDefine>& new_value){
				FLZ_LOG_INFO(FLZ_LOG_ROOT()) << "on_logger_conf_changed";
				for(auto& i : new_value) {
					auto it = old_value.find(i);
					flz::Logger::ptr logger;
					if(it == old_value.end()) {
						//新增logger
						logger = FLZ_LOG_NAME(i.name);
					} else {
						if(!(i == *it)) {
							//修改的logger
							logger = FLZ_LOG_NAME(i.name);
				...
				for(auto& i : old_value) {
					...
					if(it == new_value.end()) {
						//删除logger
						auto logger = FLZ_LOG_NAME(i.name);
