...
namespace flz {
	...
	bool ByteArray::isLittleEndian() const {
		return m_endian == FLZ_LITTLE_ENDIAN;
	}

	void ByteArray::setIsLittleEndian(bool val) {
		if(val) {
			m_endian = FLZ_LITTLE_ENDIAN;
		} else {
			m_endian = FLZ_BIG_ENDIAN;
		}
	}
...
#undef XX
	...
	bool ByteArray::writeToFile(const std::string& name) const {
		...
		if(!ofs) {
			FLZ_LOG_ERROR(g_logger) << "writeToFile name=" << name
	...
	bool ByteArray::readFromFile(const std::string& name) {
		...
		if(!ifs) {
			FLZ_LOG_ERROR(g_logger) << "readFromFile name=" << name
				<< " error, errno=" << errno << " errstr=" << strerror(errno);
			return false;
		}
