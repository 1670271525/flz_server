...
WSServer::WSServer(flz::IOManager* worker, flz::IOManager* io_worker, flz::IOManager* accept_worker)
...
void WSServer::handleClient(Socket::ptr client) {
    ...
    do {
        ...
        if(!header) {
            FLZ_LOG_DEBUG(g_logger) << "handleShake error";
            break;
        }
        WSServlet::ptr servlet = m_dispatch->getWSServlet(header->getPath());
        if(!servlet) {
            FLZ_LOG_DEBUG(g_logger) << "no match WSServlet";
            break;
        }
        int rt = servlet->onConnect(header, session);
        if(rt) {
            FLZ_LOG_DEBUG(g_logger) << "onConnect return " << rt;
            break;
        }
        while(true) {
            ...
            if(rt) {
                FLZ_LOG_DEBUG(g_logger) << "handle return " << rt;
                break;
            }
        }
